...
const generatePDFBuffer = (s: any): Promise<Buffer> => {
  return new Promise((resolve, reject) => {
    ...
    const drawHeader = () => {
      doc.fontSize(14).font('Helvetica-Bold').fillColor('black').text('P&B', 50, 40);
    ...
    if (s.signature && s.signature.startsWith('data:image')) {
      try {
        ...
        if (base64Data) {
          doc.image(Buffer.from(base64Data, 'base64'), 390, signY - 40, { width: 150 });
        }
      } catch (err) { console.error("Error firma:", err); }
    }
  ...
  try {
    ...
    if (finalize) {
      ...
      await transporter.sendMail({
        ...
        attachments: [{ filename: `Ficha_${valuesToSave.workerRut}.pdf`, content: pdfBuffer }]
      });
    }
